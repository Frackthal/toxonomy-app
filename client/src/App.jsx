...
function App() {
  ...
  const handleSearch = async () => {
    ...
    const initialTabs = Object.fromEntries(
      Object.keys(data).map(cas => [cas, Object.keys(data[cas]?.details || {})[0] || ''])
    );
  ...
  const isValidValue = (val) =>
    val && !['not classified', '-', 'not applicable', 'classification not possible'].includes(String(val).toLowerCase());
          ...
          {menuOpen && (
            <nav className="flex-1 px-4 py-4 space-y-2 text-sm">
              <button
                ...
                className="flex items-center hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded w-full text-left"
              >
        ...
        <div className="flex-1 flex flex-col overflow-auto md:ml-0">
          <header className="flex items-center justify-between px-6 h-16 min-h-16 bg-white/80 dark:bg-dm-sidebar/80 border-b dark:border-dm-border backdrop-blur-sm shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xl font-semibold text-gray-800 dark:text-white translate-y-[1px] inline-block">
                TOXONOMY
              </span>
            </div>
            <button
                ...
                {Object.entries(results).map(([cas, data]) => {
                  ...
                  return (
                    <div
                      key={cas}
                      className={
                        'bg-white dark:bg-[#bfc7d2] p-4 rounded shadow border-l-4 ' +
                        (data.sources.includes('Introuvable') ? 'border-red-600' : 'border-teal-600') +
                        ' dark:text-dm-text'
                      }
                    >
                              ...
                              <div className="w-1/4">
                                {Object.keys(details).map(table => (
                                  <div
                                    ...
                                    className={`cursor-pointer px-2 py-1 rounded mb-1 text-sm ${
                                      currentTab === table
              ...
              <div className="w-full md:w-[420px] shrink-0 space-y-4 bg-gray-100 dark:bg-[#5e6770] p-6 rounded shadow">
                ...
                <div>
                  ...
                  <div className="mb-2 space-x-2">
                    <button
                      onClick={() => setClassifications(flatOptions)}
                ...
                <div className="bg-white dark:bg-[#bfc7d2] p-4 rounded shadow dark:text-dm-text">
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={CMR_DATA}>
                      ...
                      <Tooltip
                        wrapperStyle={{
                          backgroundColor: darkMode ? '#e5e7eb' : '#fff',
                          color: '#000',
                        }}
                      />
                      <Bar dataKey="value">
                              ...
                              <div className="w-3/4 text-sm space-y-2">
                                {Object.entries(details[currentTab] || {})
                                  ...
                                  .map(([col, val]) => (
                                    <div
                                      key={col}
                                      className="bg-gray-50 dark:bg-gray-700 p-2 rounded dark:text-gray-100"
                                    >
