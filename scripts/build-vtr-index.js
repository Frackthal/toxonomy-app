#!/usr/bin/env node
// scripts/build-vtr-index.js — Uses `id` column (not rowid)
...
async function main() {
  ...
  while (true) {
    const result = await db.execute({
      sql: 'SELECT id, cas FROM vtr_all LIMIT ? OFFSET ?',
    ...
    offset += batchSize;
    if (offset % 10000 === 0) console.log(`  Progress: ${total} entries...`);
